...
int user(char *data)
{
	...
	char type[50] = "USER";
	char command[150];
	sscanf(data,"type=USER&user=%[^&]&oldpass=%[^&]&option=%c&newpass=%s", user, oldpass, &option, newpass);
	if(option == 'H') {
		//printf("%s%c%c\n","Content-Type:text/html;charset= iso-8859-1",13,10);
		printf("<html>");	
		...
		return(0);
			...
			if(WEXITSTATUS(status)) {
				...
				printf("<form><input type=\"button\" value =\"Back to menu\" onClick=\"history.go(-1);return true;\"></form><br>");		
					printf("<a href=\"http://www.cs.mcgill.ca/~dwhitn/pythonpearlsplusplus.html\">HOME</a></body></html>");
				...
				return(EXIT_FAILURE);
					...
					printf("<body>Password Changed!<br><br>");	
					printf("<form><input type=\"button\" value =\"Back to menu\" onClick=\"history.go(-1);return true;\"></form><br>");		
					printf("<a href=\"http://www.cs.mcgill.ca/~dwhitn/pythonpearlsplusplus.html\">HOME</a></body></html>");			
					return(EXIT_SUCCESS);	
					}
...
//			printf("<br /> <a href=\"http://www.cs.mcgill.ca/~gmacra/homepage\">HOME</a>");
		...
		char command[150];
		sprintf(command,"./passweb.cgi -add %s %s %s",u,p,t);
		//printf("%s", command);
		success=system(command);
		//printf("%d",WEXITSTATUS(success));
		if(WEXITSTATUS(success)==0)
		{
			printf("<br /> %s was successfully added to Members.csv!",u);
		}
		else if(WEXITSTATUS(success)==1)
		{
...
//		char au[50],ap[50],at[50];
		...
		char command[150];
		sprintf(command,"./passweb.cgi -del %s",u);
		success=system(command);
	...
	//	char au[50],ap[50],at[50];
		...
		char command[150];
		sprintf(command,"./passweb.cgi -edit %s %s %s %s %s %s",oU,oP,oT,nU,nP,nT);
		success=system(command);
		if(!success)
		{
		...
		{
			printf("<br /> could not find %s to edit or password entered incorrectly",oU);
		}
		else{
	...
	//	char oU[50], oP[50], oT[50], nU[50], nP[50], nT[50];
		...
		char command[150];
		sprintf(command,"./passweb.cgi -verify %s %s",uV,pV);
		success=system(command);
	...
	{
		printf("<html><head><title>Oops</title></head>");
		printf("<body>ERROR: One selection must be made.<br>");
		printf("<br><form><input type=\"button\" value =\"Back to menu\" onClick=\"history.go(-1);return true;\"></form>");
