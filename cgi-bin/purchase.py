...
def main ():
	if form.has_key("uname") and form["uname"].value != "" and form.has_key("password") and form["password"].value != "":
		...
		command = "./passweb.cgi -verify " + username + " " + password
		value = os.system(command)	
		if (value != 0):
			print "Login information incorrect! click below to return to catalogue. <p>"
			print "<a href=\"http://www.cs.mcgill.ca/~dwhitn/catalogue.html\">CATALOGUE</a><br/>"	
		else:
...
def purchase(username, password):
	...
	itemFour = "0"
	itemFive = "0"
	if form.has_key("item1") and form["item1"].value != "off" and form.has_key("item1A") and form["item1A"].value <> 0:
		itemOne = form["item1A"].value
	if form.has_key("item2") and form["item2"].value != "off" and form.has_key("item2A") and form["item2A"].value <> 0:
                itemTwo = form["item2A"].value
	if form.has_key("item3") and form["item3"].value != "off" and form.has_key("item3A") and form["item3A"].value <> 0:
                itemThree = form["item3A"].value
	if form.has_key("item4") and form["item4"].value != "off" and form.has_key("item4A") and form["item4A"].value <> 0:
                itemFour = form["item5A"].value
	if form.has_key("item5") and form["item5"].value != "off" and form.has_key("item5A") and form["item5A"].value <> 0:
	...
	for line in fin:
		...
		if (i == 2 and int(itemTwo) != 0 and int(row[3]) >= int(itemTwo)):
			...
			error(2)
		if (i == 3 and int(itemThree) != 0 and row[3] >= int(itemThree)):
	...
	print "<a href=\"http://www.cs.mcgill.ca/~dwhitn/pythonpearlsplusplus.html\">HOME</a><br/>"
	print "<a href=\"http://www.cs.mcgill.ca/~dwhitn/catalogue.html\">CATALOGUE</a><br/>"
...
def displayBill(price1, price2, price3, price4, price5, itemOne, itemTwo, itemThree, itemFour, itemFive):
	...
	taxedTotal = float(total) * 0.05 + float(total) * 0.075 + float(total)
...
def error(item):
	...
	print "PLEASE RETURN TO <a href=\"http://www.cs.mcgill.ca/~dwhitn/catalogue.html\">CATALOGUE<a><br/>"
